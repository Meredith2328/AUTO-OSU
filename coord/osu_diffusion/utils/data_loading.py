...
def beatmap_to_sequence(beatmap: Beatmap) -> torch.Tensor:
    ...
    for ho in hit_objects:
        try:
            data_chunks.append(get_data(ho))
        except Exception:  # noqa: BLE001 - AUTO-OSU: degenerate slider curves in a few ranked maps
            continue
    if not data_chunks:
        return torch.zeros((19, 0)).float()
...
def split_and_process_sequence(
        ...
        distance_dropout_prob: float = 0.0,
) -> tuple[tuple[torch.Tensor, torch.Tensor], int]:
    if seq.shape[1] == 0:   # AUTO-OSU: empty beatmap, the iterator moves on to the next one
        return (seq[:2, :], torch.zeros((272, 0))), 0
    ...
    # AUTO-OSU: sometimes hide the distances entirely so the model learns to choose spacing itself
    if distance_dropout_prob > 0 and random.random() < distance_dropout_prob:
        seq_d = torch.zeros_like(seq_d)
...
class BeatmapDatasetIterable:
    ...
    def __next__(self) -> tuple[any, torch.Tensor]:
        while (
            ...
            self.current_seq, self.current_seq_len = split_and_process_sequence(
                beatmap_to_sequence(beatmap), double_time, self.args.distance_std, self.args.distance_dropout_prob)
