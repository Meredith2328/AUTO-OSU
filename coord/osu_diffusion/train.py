...
from osu_diffusion.utils.models import DiT_models
from osu_diffusion.utils.diffusion import create_diffusion
from osu_diffusion.utils.data_loading import get_data_loader
...
def update_ema(ema_model, model, decay=0.9999):
    ...
    for name, param in model_params.items():
        name = name[7:] if name.startswith("module.") else name   # AUTO-OSU: DDP-wrapped model under accelerate
...
def main(args):
    ...
    accelerator.init_trackers(
        os.environ.get("WANDB_PROJECT", "autoosu-coords"),
        init_kwargs={
            "wandb": {
                "name": os.environ.get("WANDB_RUN_NAME", "coord"),
                "job_type": "training",
                "config": dict(args),
